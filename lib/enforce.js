 ...
 */
const findTrailingCommentInImport = (node) => {
  let trailingCommentBeforeEnd = null;
  const [, nodeEndPos] = node.range;
  ...
  node.parent.comments.some((comment) => {
    ...
    if (commentStartPos > nodeEndPos) {
      // Comments after the node are ignored
      return true;
    }
...
const fixer = (node, semi, spacer = '\n') => (eslintFixer) => {
  const trailingComment = findTrailingCommentInImport(node);
  let defaultImport = '';
  let namespaceImport = '';
  const objectImports = [];
  ...
  const newValue = `${importKeyword} ${defaultImportValue}${namespaceImportValue}${objectImportsValue} from ${node.source.raw}${outputComment(trailingComment)}${semi || trailingComment ? ';' : ''}`;
  ...
  create(context) {
    ...
    return {
      ImportDeclaration(node) {
        ...
        const importLineCount = 1 + (node.source.loc.end.line - node.loc.start.line);
